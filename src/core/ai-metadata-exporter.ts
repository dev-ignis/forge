...
export class AIMetadataExporter {
  ...
  static exportTrainingDataset(
    ...
    components.forEach((metadata, tagName) => {
      ...
      const trainingData = this.generateComponentTrainingData(
        tagName, 
        metadata,
        { includeAntiExamples, includeRelationships }
      );
...
`).join('\n')}`;
  ...
  private static generateGlobalPatterns(
    ...
    // Form patterns
    const formComponents = Array.from(components.entries())
  ...
  <${formComponents.find(([_, m]) => m.semanticRole === 'button')?.[0] || 'forge-button'} type="submit">Submit</forge-button>
    ...
    // Modal patterns
    const modalComponents = Array.from(components.entries())
      .filter(([_, metadata]) => metadata.usagePatterns.includes('modal triggers'));
...
### Code Examples
${Object.entries(comp.metadata.codeExamples).map(([framework, code]) => `
...
`).join('\n')}`;
  ...
  private static formatForClaudeArtifacts(dataset: AITrainingDataset): string {
    const claudeData = {
      ...
      components: dataset.components.reduce((acc, comp) => {
        acc[comp.tagName] = {
...
Always include proper ARIA attributes, keyboard navigation support, and follow the design system tokens.`;
  ...
  private static generatePatternPrompts(components: Map<string, AIComponentMetadata>): Record<string, string> {
...
${Array.from(components.entries())
  ...
  private static generateFrameworkPrompts(): Record<string, string> {
    return {
      react: 'For React integration, import from "@nexcraft/forge/integrations/react" and use standard React patterns for props and events.',
      vue: 'For Vue integration, use kebab-case component names and Vue event syntax with @ symbols.',
      angular: 'For Angular integration, import CUSTOM_ELEMENTS_SCHEMA and use Angular event binding syntax.'
    };
  }
